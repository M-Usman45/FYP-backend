...
router.post("/checked/:id", async (req, res) => {
  ...
  const asset = Asset.findOneAndUpdate({title:req.body.title},{$inc:{quantity:1}},
       ...
       const checkIns = await CheckIn.findOneAndDelete({_id: req.params.id})
